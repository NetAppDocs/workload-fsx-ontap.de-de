...
* *Host mit Failover-Knoten verbinden*
+
Workload Factory empfiehlt, Hosts mit dem Failover-Knoten zu verbinden, um die Konnektivität aufrechtzuerhalten und Unterbrechungen während Failover-Vorgängen zu vermeiden.
...
Leistungseffizienz konzentriert sich auf die effiziente Nutzung von IT- und Computerressourcen.

* *Host mit bevorzugtem Node verbinden*
+
In einem hochverfügbaren FSx for ONTAP Bereitstellungsmodell sollten Hosts direkt mit dem Knoten verbunden sein, der das Blockgerät bereitstellt, um direkten Zugriff und optimale Leistung sicherzustellen.
