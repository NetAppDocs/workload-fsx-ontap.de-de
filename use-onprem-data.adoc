...
summary: Lokale ONTAP Daten in der NetApp Workload Factory erkennen und replizieren, sodass sie als Backup in der Cloud genutzt werden können. 
---
...
[role="lead"]
Lokale ONTAP Daten in der NetApp Workload Factory erkennen und replizieren, sodass sie als Backup in der Cloud genutzt werden können.
...
* Eine verbundene Verbindung, die mit dem erkannten lokalen Cluster verknüpft wird. Wenn keine Verbindung vorhanden ist, ist es erforderlich, link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html["Erstellen Sie eine"^].
...
.. *ONTAP-Zugangsdaten*: Bitte geben Sie Folgendes an:
+
*** GovCloud accounts: Geben Sie die ARN mit ONTAP Zugangsdaten ein.
*** Alle anderen Konten: Geben Sie die ONTAP-Anmeldeinformationen für den lokalen ONTAP Cluster ein. Stellen Sie sicher, dass der Benutzer über die erforderlichen Berechtigungen verfügt.
...
* Sowohl der lokale ONTAP Cluster als auch das FSx for ONTAP Dateisystem, das Sie für die Replikationsbeziehung verwenden, müssen über eine zugeordnete Verbindung verfügen. link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html["Erfahren Sie, wie Sie einen vorhandenen Link zuordnen oder einen neuen Link erstellen und zuordnen."^]. Nachdem die Verbindung zugeordnet wurde, kann zu diesem Vorgang zurückgekehrt werden.
